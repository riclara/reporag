...
- When Codex creates a branch/commit/PR flow, treat the PR title as release-critical metadata and set it in Conventional Commit format before sharing the PR.
- Branch names do not affect `release-please`; prioritize correcting the PR title over renaming the branch.
- Prefer PR titles like `fix(scope): summary` or `feat(scope): summary`. Use `chore(main): release ...` only for release PRs generated by automation.
- If Codex also creates the local commit, prefer using the same Conventional Commit style there so local history and the eventual squash title stay aligned, even though the PR title is the source of truth for `main`.
